...
- [!] T010 [P2] 编写 Docker 部署配置
  ...
  Notes: 已新增 Dockerfile、docker-compose.yml、VPS env 示例、README 和 GitHub Actions SSH 部署 workflow；`docker compose config` 通过。本机 Docker daemon 未运行，无法完成容器启动后的 `/health` 验证。
