...
- [x] T010 [P2] 编写 Docker 部署配置
  Goal: 支持 Runtime 只读部署和 Admin 维护部署。
  Notes: 已新增 Dockerfile、docker-compose.yml、VPS env 示例、README 和 GitHub Actions SSH 部署 workflow；`docker compose config` 通过；本地 compose 启动后 `/health` 正常。
